...
class OutputWeek
  # arguments: google auth token, week (starting at 1)
  def self.write(token, week)
    ...
    teams.each do |t|
      ...
      while !sheet[y, 1].blank? || emen do
        ...
        sheet[y, week + 1] = p.game_stats.find_by(week: week).points if !p.blank? && sheet[y, week + 1].blank?
        y += 1
      end
      sheet.save
      p "finished #{t.name}"
    ...
    # TODO change to -1
    team_games = TeamGame.where(game_id: Game.where(week: week-2).pluck(:id))
    x = 1
    y = 1
    # TODO since my schedule is delayed, change to 1
    time = Game.find_by(week: week-2).time.to_s(:slashes)
    ...
    Team::STD_SIZE.times do
      ...
      if !team.blank?
         sheet[y, x] = "#{team_name} (#{team_games.find_by(team_id: team.id).points})"
      end
      x = x > 2 ? 2 : 3
      y += 1 if x > 2
    end
